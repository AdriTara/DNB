...
for line in open(sys.argv[3]): #'datos_trabajo/ortologos-Nt_At.txt' #sys.argv[3]
    ...
    AT = line.split('\t')[1]
    NT_AT[NT] = AT
print('Ortologous relations are obtained', datetime.datetime.now())
...
for core in resultado:
    ...
    lexpre += core[2]
    probes += core[1]
print('Expression data taken', datetime.datetime.now())
ntID = []
atID = []
for probe in probes:
    try:
...
print('Mean calculations ---OK\nExpression vectors---OK')

FCs = medias[medias.columns[:-1]].copy()
for column in FCs.columns[3:]:
    FCs.loc[:,column] = np.log2((medias[column]/medias['Negative']).astype(np.float64))

#%% Biphasic analysis
print('Biphasic analysis starts', datetime.datetime.now())
...
#Order biphasic genes in DeltaFC decreasing order
bifasicos = bifasicos.reindex(bifasicos['DeltaFC'].abs().sort_values(ascending = False).index)
bifasicos = bifasicos.set_index('atID')

#Separate biphasic genes depending on their biphasic increment
bifasicos_up = bifasicos[bifasicos['DeltaFC'] > 0]
bifasicos_down = bifasicos[bifasicos['DeltaFC'] < 0]
print('Biphasic analysis---OK', datetime.datetime.now())

#%% Module obtention
print('Module obtention starts', datetime.datetime.now())
interacciones = []
i = 0
...
for gen in bifasicos.index.tolist():
    i += 1
    if i % 1000 == 0:
        print('Submodules creation keeps going', gen)
    for interaccion in interacciones:
        if (gen == interaccion[0]) or (gen == interaccion[1]):
            ...
            if gen in submodulos:
                ...
                #Check that if belongs to other submodule
...
for submodulo1 in sorted(submodulos):
    if i % 100 == 0:
        print('Module creation keeps going', submodulo1)
    try:
        i += 1    
...
print('Modules are created', datetime.datetime.now())
#%% CI calculation
print('CI calculation start', datetime.datetime.now())
...
print('Expression vector FC---OK ', datetime.datetime.now())
dfs_modulos = {}
for modulo in modulos:
    ...
    for gen in modulos[modulo]:
        try:
            ...
            continue
print('Module dataframes are obtained', datetime.datetime.now())
...
#%% OUTPUTS
print('Output generation starts', datetime.datetime.now())
...
for modulo in dfs_modulos:
    ...
    plt.clf()
            #break
print('Outputs are generated\nEnd of the analysis', datetime.datetime.now())